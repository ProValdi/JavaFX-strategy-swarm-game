...
import chatgptonly.entities.units.Entity;
...
import static chatgptonly.entities.buildings.PlaceMode.BARRACKS;
import static chatgptonly.entities.buildings.PlaceMode.NONE;

public class DynamicRenderer {

    // вызывается каждый кадр
    public void redrawDynamic(GraphicsContext gc, World world,
        ...
        if (placeMode != NONE && hoverTileX >= 0 && hoverTileY >= 0) {
            ...
            gc.fillRect(hoverTileX * TILE_SIZE, hoverTileY * TILE_SIZE, w * TILE_SIZE, h * TILE_SIZE);

            gc.setStroke(Color.WHITE);
            gc.strokeRect(hoverTileX * TILE_SIZE, hoverTileY * TILE_SIZE, w * TILE_SIZE, h * TILE_SIZE);
        }
    ...
    private void drawHud(GraphicsContext gc, PlaceMode mode) {
        ...
        gc.fillRect(SCREEN_W - 180, 10, 170, 70);

        gc.setFill(Color.WHITE);
        gc.fillText("1 - Drill", SCREEN_W - 170, 35);
        gc.fillText("2 - HeatGen", SCREEN_W - 170, 55);
        gc.fillText("3 - Barracks", SCREEN_W - 170, 75);
        gc.setFill(Color.LIGHTGREEN);
        gc.fillText("Mode: " + mode, SCREEN_W - 170, 95);
    }

    private void drawBarracksPanel(GraphicsContext gc) {
        double x = SCREEN_W - 180;
        double y = 110;
        double w = 170, h = 90;
